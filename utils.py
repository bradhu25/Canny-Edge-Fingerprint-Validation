...
Final Project
Author: Bradley Hu (bradleyh@stanford.edu)
...
def conv(image, kernel):
    ...
    for n in range(Hi):
        for m in range(Wi):
            out[n, m] = np.sum(padded[n:n + Hk, m:m + Wk] * flipped)
    ### END YOUR CODE
    ...
    for i in range(1, H+1):
        for j in range(1, W+1):
            ...
            if angle == 0 or angle == 180:
                candidates = (G[i, j-1], G[i, j+1])
            elif angle == 45 or angle == 225:
                candidates = (G[i-1, j-1], G[i+1, j+1])
            elif angle == 90 or angle == 270:
                candidates = (G[i-1, j], G[i+1, j])
            elif angle == 135 or angle == 315:
                candidates = (G[i+1, j-1], G[i-1, j+1])
            
            if G[i,j] > candidates[0] and G[i,j] > candidates[1]:
                out[i-1, j-1] = G[i, j]
            else:
                out[i-1, j-1] = 0
...
def link_edges(strong_edges, weak_edges):
    ...
    for (i, j) in indices:
        ...
        while queue:
            node = queue.pop(0)
            for neighbor in get_neighbors(node[0], node[1], H, W): 
                if weak_edges[neighbor[0], neighbor[1]] and not edges[neighbor[0], neighbor[1]]:
                    edges[neighbor[0], neighbor[1]] = 1
